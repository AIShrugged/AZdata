...
**HS commodity code (EQM).** Pure embedding search couldn't bridge product names → formal HS nomenclature. The approach is **LLM-first**: predict the likely **HS heading**, filter the **9,957-code registry** to it (now **unioned** with embedding retrieval so a wrong heading can't drop the true code), then rerank. **HS accuracy is now measured** (`scripts/eval_eqm.py` + a starter gold set) rather than asserted — and it is the **weakest part of the system**: on the starter gold (n=8) it scores **~38% heading-level / 50% candidate-recall**, with candidate retrieval as the bottleneck (the AZ product name often doesn't surface the right heading). The correctness bugs the audit found are fixed (heading union; confidence now describes the code actually returned and a non-numeric confidence no longer discards it). HS-code assignment needs further work and a domain-expert-labelled gold set.
