...
- **⚠️ Evaluation integrity (under re-validation).** A code audit ([`docs/AUDIT.md`](docs/AUDIT.md)) found the train/dev/test split was **not deduplicated by text**, so **~20% of dev/test items appear verbatim in the train-based RAG retrieval index with their gold labels** (measured: 19.5% dev / 20.2% test). This **inflates every Task-2 number above — micro *and* macro** — so they are **not a valid estimate of generalization** until the data is re-split (text/source-level dedup, zero train↔eval overlap asserted) and the eval is re-run. Treat current Task-2 accuracy as **pending re-validation**.
